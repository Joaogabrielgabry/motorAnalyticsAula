# ============================================================
#  queries/aggregation.py
#  Todas as 12 queries do enunciado
# ============================================================
...
def _ts_to_secs(ts):
    """'YYYY-MM-DD HH:MM:SS' → segundos desde época (aproximado para comparações)."""
    d   = ts[:10]
    hms = ts[11:]
...
def _filter_interval(events, ts_ini, ts_fim):
    """Filtra lista de eventos por intervalo de timestamp (string compare funciona p/ ISO 8601)."""
    return [e for e in events if ts_ini <= e.timestamp <= ts_fim]

def _sqrt(n):
    """Raiz quadrada inteira (sem math)."""
    if n <= 0: return 0.0
    x = float(n)
    for _ in range(50):
        x = (x + n / x) / 2
...
def q2_media_permanencia(events, idx_zone, zones):
    ...
    # horas de operação 9..20
    horas = list(range(9, 21))

    # acumular durações por (zona, hora)
    acc = HashTable(size=512)   # "zona|hora" → [durations]
...
def q3_picos_ocupacao(events, idx_date):
    ...
    # sweep line: lista de (timestamp, +1 ou -1)
    sweep = []
    for ev in evs_dia:
        if ev.event_type == "entry":
    ...
    # top-k momentos de maior ocupação
...
def q5_topk_zonas(events, zones):
    ...
    pairs  = list(cnt.items())           # (zone_id, contagem)
...
def q6_topk_blocos(events, zones):
    ...
    cnt = HashTable(size=2048)   # "YYYY-MM-DD|HH|MM" arredondado a 30 min
...
def q8_fluxo_entre_zonas(events, zones):
    ...
    # ordenar todos os eventos por (gender, age, timestamp)
...
def q9_sequencias_zonas(events):
    ...
    # reconstruir sequência global de zones (apenas entry, ordem temporal)
...
def q10_anomalias(events, zones):
    ...
    # contar entradas por (zona, dia, hora)
    cnt = HashTable(size=4096)   # "zona|dia|hora" → count
    dias_por_zona_hora = HashTable(size=2048)  # "zona|hora" → set de dias
    ...
    # calcular médias e desvios por (zona, hora)
    stats = HashTable(size=2048)   # "zona|hora" → (mean, std, [counts])

    # primeiro recolher todos os counts
